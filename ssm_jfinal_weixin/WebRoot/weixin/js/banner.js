    ...
    iScroll.prototype = {
        ...
        _start: function(e) {
            ...
            var that = this,
        		...
        		if (that.wrapperW <= 1){that.refresh();}
            if (!that.enabled)
                return;
...
function loaded() {
 setInterval(function() {
      if(tt_resize){fnresize();myScroll.refresh();tt_resize=false;}
      myScroll.scrollToPage('next', 10, 400, is_first);			     
  }, 3500);
 fnresize();	
  myScroll = new iScroll('wrapper', {
      ...
      onScrollEnd: function() {
      	  if(this.currPageX ==5)this.currPageX =-1
      	  if(document.querySelector('#indicator > li.active')!=null)
          	document.querySelector('#indicator > li.active').className = '';

          if(document.querySelector('#indicator > li:nth-child(' + (this.currPageX + 1) + ')')!=null) 
 ...
 var is_first =2;
 function fnresize(){
	var cw = $("body")[0].clientWidth
	var ch = document.documentElement.clientHeight
	for (i = 0; i < is_first; i++) {
      document.getElementById("thelist").getElementsByTagName("div").item(i).style.width = cw + "px";
      document.getElementById("thelist").getElementsByTagName("div").item(i).style.height = parseInt(ch * 0.99)  + "px";
	}
	document.getElementById("scroller").style.width = cw * is_first + "px";
	document.getElementById("scroller").style.height = parseInt(ch * 0.99)  + "px";
 }
 
 function doscrollTopage(page){
	 myScroll.currPageX = page;
	 if(page >0)myScroll.currPageX = page+1;
	 if(page > myScroll.currPageX){
