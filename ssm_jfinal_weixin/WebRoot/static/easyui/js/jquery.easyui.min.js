...
$(_4e).unbind(".linkbutton").bind("focus.linkbutton",function(){
if(!_4f.disabled){
...
$("<td><a href=\"javascript:void(0)\" icon=\"pagination-first\" title=\"前进到第一页\"></a></td>").appendTo(tr);
$("<td><a href=\"javascript:void(0)\" icon=\"pagination-prev\" title=\"前进到上一页\"></a></td>").appendTo(tr);
$("<td><div class=\"pagination-btn-separator\"></div></td>").appendTo(tr);
$("<span style=\"padding-left:6px;\"></span>").html(_5c.beforePageText).wrap("<td></td>").parent().appendTo(tr);
$("<td><input class=\"pagination-num\" type=\"text\" value=\"1\" size=\"2\"></td>").appendTo(tr);
$("<span style=\"padding-right:6px;\"></span>").wrap("<td></td>").parent().appendTo(tr);
$("<td><div class=\"pagination-btn-separator\"></div></td>").appendTo(tr);
$("<td><a href=\"javascript:void(0)\" icon=\"pagination-next\" title=\"前进到下一页\"></a></td>").appendTo(tr);
$("<td><a href=\"javascript:void(0)\" icon=\"pagination-last\" title=\"前进到最后一页\"></a></td>").appendTo(tr);
if(_5c.showRefresh){
$("<td><div class=\"pagination-btn-separator\"></div></td>").appendTo(tr);
$("<td><a href=\"javascript:void(0)\" icon=\"pagination-load\" title=\"刷新当前页面的数据\"></a></td>").appendTo(tr);
}
//添加打印当前页的按钮
if(_5c.showPrint){
$("<td><div class=\"pagination-btn-separator\"></div></td>").appendTo(tr);
$("<td><a href=\"javascript:void(0)\" icon=\"pagination-print\" title=\"打印当前页面的数据\"></a></td>").appendTo(tr);
$("<td><a href=\"javascript:void(0)\" icon=\"pagination-export\" title=\"导出当前页面的数据到Excel文件\"></a></td>").appendTo(tr);
}
if(_5c.buttons){
$("<td><div class=\"pagination-btn-separator\"></div></td>").appendTo(tr);
...
_5d.find("a[icon=pagination-print]").unbind(".pagination").bind("click.pagination",function(){
if(typeof dg=="string"){dg=$('#'+dg)}
DatagridPagePrint('打印数据',dg);
});
_5d.find("a[icon=pagination-export]").unbind(".pagination").bind("click.pagination",function(){
if(typeof dg=="string"){dg=$('#'+dg)}
DatagridPageExport('第'+_5c.pageNumber+'页数据',dg);
});
...
function _68(_69){
	if($.data(_69,"pagination")==undefined){return}
...
$.fn.pagination=function(_71,_72,_dg){
...
$.fn.pagination.defaults={total:1,pageSize:10,pageNumber:1,pageList:[10,20,30,50],loading:false,buttons:null,showPageList:false,showRefresh:true,showPrint:true,onSelectPage:function(_75,_76){
...
//add by yyj 2016-01-28 增加对树节点text的查找	
$("<span class=\"tree-title\"></span>").html(_ce.text).attr("node-text", (_ce.text.indexOf("<a") > 0 ? _ce.text.substr(0, _ce.text.indexOf("<a")) : _ce.text)).appendTo(_cf);
if(_c6.checkbox){
...
//add by yyj 2016-01-28 增加对树节点text的查找
node.find(".tree-title").attr("node-text", (data.text.indexOf("<a") > 0 ? data.text.substr(0, data.text.indexOf("<a")) : data.text)).html(data.text);
if(data.iconCls){
...
if(node.length){
	...
	//add by yyj 2016-01-28 增加对树节点text的查找
	node=$(_121).find("div.tree-node").find(".tree-title[node-text^="+id+"]");
	if(node.length){
		return _b1(_121,node.parent()[0]);
	}else{
		return null;
	}
}
...
if($.data(_17d,"panel").options==undefined){return}
...
function _1de(_1df,_1e0,_1e1){
	//判断是否已经打开了一个提示对话框
if($('body').find(".messager-icon")[0]!=undefined){
	//return 已设置按钮焦点，不需要判断
}
...
win.window({title:_1df,width:400,height:"auto",closable:true,modal:true,collapsible:false,minimizable:false,maximizable:false,resizable:false,onClose:function(){
setTimeout(function(){
win.window("destroy");
},100);
}});
...
switch(icon){
case "error":
_1e5="<div id=\"_alert\" class=\"messager-icon messager-error\"></div>"+_1e5;
break;
case "info":
_1e5="<div id=\"_alert\" class=\"messager-icon messager-info\"></div>"+_1e5;
break;
case "question":
_1e5="<div id=\"_alert\" class=\"messager-icon messager-question\"></div>"+_1e5;
break;
case "warning":
_1e5="<div id=\"_alert\" class=\"messager-icon messager-warning\"></div>"+_1e5;
break;
}
_1e5+="<div style=\"clear:both;\"/>";
var _1e6={};
_1e6[$.messager.defaults.ok]=function(){
win.window("close", true); //强制退出，不经过onbeforeclose
if(fn){
fn();
return false;
}
...
/*yyj confirm回调函数在onBeforeClose中执行，并传入false，单击按钮close窗口的，需要跳过onBeforeClose
*/
win.window({onBeforeClose:function(){if(fn){setTimeout(function(){fn();},0)};return true}})
},confirm:function(_1e7,msg,fn){
var _1e8="<div id=\"_confirm\" class=\"messager-icon messager-question\"></div>"+"<div>"+msg+"</div>"+"<div style=\"clear:both;\"/>";
var _1e9={};
...
/*yyj confirm回调函数在onBeforeClose中执行，并传入false，单击按钮close窗口的，需要跳过onBeforeClose
*/
win.window({onBeforeClose:function(){if(fn){setTimeout(function(){fn(false);},0)};return true}})  
},prompt:function(_1ea,msg,fn){
var _1eb="<div class=\"messager-icon messager-question\"></div>"+"<div>"+msg+"</div>"+"<br/>"+"<input id=\"promptip\"  class=\"messager-input\" type=\"text\"/>"+"<div style=\"clear:both;\"/>";
var _1ec={};
...
pp.panel($.extend({},_257.options,{iconCls:(_257.options.icon?_257.options.icon:undefined)})); //tab
var opts=pp.panel("options");
var tab=opts.tab;
...
//yyj改写，将tab的close操作分解成onBeforeClose及之前的为一部分，余下的为另一个函数
function _249_new(_25a,_25b){
var opts=$.data(_25a,"tabs").options;
var tabs=$.data(_25a,"tabs").tabs;
var _25c=$.data(_25a,"tabs").selectHis;
...
},closeNoBefore:function(jq,_274){
return jq.each(function(){
_249_new(this,_274);
});
...
if($.data(jq[0],"layout")==undefined){return}
...
function _395(_396,_397,_38e){ //*   
if($.data(_396,"datagrid")==undefined){return}
...
//yyj easyui在这里没有对被设置了height的tr进行原属性的保留，c1对应的是rownumbers表，该表是否存在取决于datagrid的rownumbers属性是否为true，固应该以c2为准
var tr1h = tr1.height();
var tr2h = tr2.height();
tr1.css("height","");
tr2.css("height","");
var _3a2=Math.max(tr1.height(),tr2.height());
if (isroot==false){ //foot不按rowstyles的高度走
	if (tr2h > 0){_3a2 = tr2h}
}
//yyj,IE进行特殊处理
if(getwebType().ie){_3a2 = _3a2 - 2}
tr1.css("height",_3a2);
tr2.css("height",_3a2);
};
};
...
//重写，更改标题的对齐方式为：优先取自定义的属性titlealign，没有就居中
cell.css("text-align",(col.titlealign||"center"||col.align||"left"));
}else{
...
_3d7.find("div.datagrid-cell").unbind(".datagrid").bind("click.datagrid",function(event){
//重写排序，点击帮助图标不进行排序
var event=event||window.event
var de=event.srcElement||event.target;
if(de){
	if(de.tagName.toUpperCase()=="IMG"){return;}
}
...
function _3fa(_3fb){
	if($.data(_3fb,"datagrid")==undefined){return}
...
function _406(_407,data){//data
	if($.data(_407,"datagrid")==undefined){return}
...
_40c.pagination({total:data.total},_407.id,_407.id);//yyj
}
}
_395(_407);
_3c2(_407);
...
for(var i=0;i<rows.length;i++){
if(_40d(rows[i])){
_427(_407,rows[i][opts.idField]); //selectrecord
}
}
}
function _40d(row){ //selectrow
...
function _40e(_40f,row){//* getrowindex
...
function _410(_411){         //getselected,getselections
...
function _415(_41c){      //unselectall
	if($.data(_41c,"datagrid").options==undefined){return}
...
function _3d2(_420,_421){    //selectrow
...
function _427(_428,_429){       //selectRecord
...
function _3d3(_42b,_42c){     //unselectRow
...
function _42f(_430,_431){     //beginEdit
...
function _435(_436,_437,_438){      //cancelEdit,Edit
...
function _440(_441,_442){     //getEditors
var _443=[];
...
function _445(_446,_447){      //getEditor
...
//yyj，改写，easyui很傻瓜，什么时候rows的某一行改变高度了，foots对应行也一定要跟着改变高度
_395(_449,_44a,0);
};
...
function _46a(_46b){
	if($.data(_46b,"datagrid")==undefined){return}
...
}},numberbox:{init:function(_49e,_49f){ //editor numberbox 限定输入长度
var _4a0=$("<input maxlength=\"15\" type=\"text\" class=\"datagrid-editable-input\">").appendTo(_49e);
_4a0.numberbox(_49f);
return _4a0;
...
$.fn.datagrid.methods={options:function(jq){ //datagrid 定义默认所有方法
...
return $.data(jq[0],"datagrid").panel;
	...
	if($.data(jq[0],"datagrid")==undefined){return}
...
//yyj
_4d5.pagination({pageNumber:1},$(this),$(this));
_476(this,_4d4);
});
...
return jq.each(function(){
	if($(this).datagrid("getPager")==undefined){return null}
$(this).datagrid("getPager").pagination("loaded");
var _4da=$(this).datagrid("getPanel");
_4da.children("div.datagrid-mask-msg").remove();
_4da.children("div.datagrid-mask").remove();
});
},fitColumns:function(jq){ //重写fitcolumns，以适应拖动自定义列
return jq.each(function(){
_3b2_1(this); //重新布局datagrid,不需要重新载入
_3e1(this);   //datagrid.cell 自适应大小
_3d4(this);     //重新绑定header的事件
});
...
},refreshRow:function(jq,_4e3){//已验证
return jq.each(function(){
var opts=$.data(this,"datagrid").options;
...
},rebindEvent:function(jq){
	//此函数为自定义，用于重写绑定datagrid的事件
return jq.each(function(){
_3d4(this);
});
...
for(var i=0;i<rows.length;i++){
//var cls=(i%2&&opts.striped)?"class=\"datagrid-row-alt\"":"";
//var _4f5=opts.rowStyler?opts.rowStyler.call(_4f7,i,rows[i]):"";
//var _4f6=_4f5?"style=\""+_4f5+"\"":"";	
...
$(_4f8).html(_4fb.join(""));
},renderRow:function(_4fc,_4fd,_4fe,_4ff,_500){ //datagrid 数据写入
...
return cc.join("");
},refreshRow:function(_505,_506){//已验证
...
},insertRow:function(_50c,_50d,row){ //已验证
...
},onBeforeRender:function(_518,rows){//验证
...
},doPagination:function(pPageIndex,pPageSize){//添加datagrid本地数据加载分页
}});
})(jQuery);
(function($){
function _53d(_53e){
...
opts.pageSize=_56a;
if(opts.url==null || opts.url==""){ //非本地数据不进行分页
opts.doPagination(_569,_56a);
}
_56b(_561);
}},_561.id,_561.id);//yyj
...
_594.pagination({total:data.total},_58d.id,_58d.id);	//yyj
}
}
_56c(_58d);
_57a(_58d);
...
return cc.join("");
},refreshRow:function(_604,id){//已验证
...
$(_6c9).combo("setValue",opts.formatter(_6ca.calendar.calendar("options").current)).combo("setText",opts.formatter(_6ca.calendar.calendar("options").current));
$(_6c9).combo("hidePanel");
});
