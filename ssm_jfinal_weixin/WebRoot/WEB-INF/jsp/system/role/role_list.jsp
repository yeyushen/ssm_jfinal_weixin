...
<body class="no-skin">
						...
						<div class="col-xs-12">
							...
							<table>
								...
								<a class="btn btn-mini btn-info" onclick="editRole('${pd.ROLE_ID }');">修改组名称<i class="icon-arrow-right  icon-on-right"></i></a>
									...
									<c:choose>
										...
										<a class="btn btn-mini btn-purple" onclick="editWarningNewsRights('${pd.ROLE_ID }');"><i class="icon-pencil"></i>组预警消息权限</a>
										</c:if>
										</c:otherwise>
									</c:choose>
									<c:choose> 
							...
							<table id="dynamic-table" class="table table-striped table-bordered table-hover" style="margin-top:7px;">
								...
								<tr>
									...
									<th style="width:250px;"  class="center">操作</th>
								</tr>
								</thead>
								<c:choose>
									<c:when test="${not empty roleList_z}">
										...
										<a class="btn btn-mini btn-purple" onclick="editWarningNewsRights('${var.ROLE_ID }');"><i class="icon-pencil"></i>预警消息权限</a>
	...
	<script type="text/javascript">
		...
		//菜单权限
		function editWarningNewsRights(ROLE_ID){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag = true;
			 diag.Title = "消息权限";
			 diag.URL = '<%=basePath%>role/newsqx.do?ROLE_ID='+ROLE_ID;
			 diag.Width = 320;
			 diag.Height = 450;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
