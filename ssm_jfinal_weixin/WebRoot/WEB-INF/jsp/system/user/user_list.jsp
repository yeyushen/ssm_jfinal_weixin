								...
								<c:when test="${not empty userList}">
									...
									<c:forEach items="${userList}" var="user" varStatus="vs">
										...
										<tr>
											...
											<td class="center">
											<c:if test="${user.nick_name!=null }"><a title="代理会员" class="btn btn-mini btn-warning" >代理会员</a></c:if>
											<c:if test="${user.nick_name==null }"><a title="系统用户" class="btn btn-mini btn-info" >系统用户</a></c:if>
											</td>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<a class="btn btn-xs btn-info" title="编辑" onclick="editUser('${user.USER_ID}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													</c:if>
													<c:if test="${user.nick_name!=null }">
													<a class="btn btn-xs btn-warning" title="编辑" onclick="editPUser('${user.USER_ID}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													</c:if>
													</c:if>
					...
					<table style="width:100%;">
						<tr>
							<td style="vertical-align:top;">
								...
								<%-- <c:if test="${QX.add == 1 }">
								<a class="btn btn-mini btn-warning" onclick="addProxy();">新增会员代理</a>
								</c:if> --%>
...
function addProxy(){
	 ...
	 diag.Drag=true;
	 diag.Title ="新增代理会员";
	 diag.URL = '<%=basePath%>user/goAddPU.do';
	 diag.Width = 500;
	 diag.Height = 600;
	 diag.CancelEvent = function(){ //关闭事件
		 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
...
//修改
function editPUser(user_id){
	 top.jzts();
	 var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="资料";
	 diag.URL = '<%=basePath%>user/goEditPU.do?USER_ID='+user_id;
	 diag.Width = 500;
	 diag.Height = 600;
	 diag.CancelEvent = function(){ //关闭事件
		 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
