						...
						<div class="col-xs-12">
								<form action="user/${msg }.do" name="userForm" id="userForm" method="post">
									...
									<table id="table_report" class="table table-striped table-bordered table-hover">
										...
										<tr>
											<td style="width:75px;text-align: right;padding-top: 13px;">代理会员:</td>
											<td id="proxyopenid">			
											<select class="chosen-select form-control" name="OPENID" id="openid" data-placeholder="新增或编辑系统用户则不用选择此处!" maxlength="255" placeholder="这里输入代理会员" title="代理会员" style="width:98%;">
														<option value=""></option>
													<c:forEach items="${memberList}" var="member">
														<option <c:if test="${pd.OPENID==member.openid }">selected</c:if> value="${member.openid }">${member.nick_name }</option>
													</c:forEach>
												</select>
											</td>
										</tr>
										...
										<tr>
										<td style="width:79px;text-align: right;padding-top: 13px;">是否锁定:</td>
										<td>
											<select class="form-control" name="is_lock" id="is_lock" style="vertical-align:top;" style="width:98%;" >
											    <option value="0" selected>正常</option>
												<option value="3" <c:if test="${pd.last_try_count >= 3 }">selected</c:if>>已锁定</option>
											</select>	
										</td>									
										</tr>
										<tr>
