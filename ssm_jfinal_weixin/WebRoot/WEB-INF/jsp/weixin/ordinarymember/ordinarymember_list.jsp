...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="ordinarymember/list.do" method="post" name="Form" id="Form">
						<table style="margin-top:5px;">
							<tr>
								...
								<td>
									<div class="nav-search">
										<span class="input-icon">
											<input type="text" placeholder="输入最低钱包金额" class="nav-search-input" id="min" name="min" autocomplete="off" value="${page.pd.min }" onblur="validatemin()" />	
										</span>
									</div>
								</td>
								<td>
									<div class="nav-search">
										<span class="input-icon">
											<input type="text" placeholder="输入最高钱包金额" class="nav-search-input" id="max" name="max" autocomplete="off" value="${page.pd.max }" onblur="validatemax()" />
										</span>
									</div>
								</td>
								...
								<c:if test="${QX.cha == 1 }">
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>
											    <button type="button" class="btn btn-link" onclick="showCommission('${var.openid}');">
											    ${var.name}
											    </button>
											</td>											
											...
											<td class='center'>
											<fmt:formatDate value="${var.create_date}" type="both" pattern="yyyy-MM-dd HH:mm"/>
											</td>
	...
	$(document).keyup(function(event){
		...
		function showCommission(openId){
			 ...
			 diag.Drag=true;
			 diag.Title ="会员佣金明细";
			 diag.URL = '<%=basePath%>ordinarymember/goCommission.do?open_id=' + openId;
			 diag.Width = 700;
			 diag.Height = 600;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
