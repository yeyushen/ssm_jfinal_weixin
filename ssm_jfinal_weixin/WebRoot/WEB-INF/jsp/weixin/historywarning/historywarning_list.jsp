...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="historywarning/list.do" method="post" name="Form" id="Form">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<th class="center">预警会员昵称</th>
									<th class="center">预警会员姓名</th>
							...
							<c:choose>
								...
								<c:if test="${QX.cha == 1 }">
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' value="${var.warnrec_id}" class="ace" /><span class="lbl"></span></label>
											</td>
											...
											<td class='center'>
											 <button type="button" class="btn btn-link" onclick="edit('${var.openid}');">${var.name}</button>
											</td>											
											<td class='center'>${var.content}</td>
											<td class='center'>${var.ts}</td>
											<td class="center">
												...
												<c:if test="${QX.del == 1}">
													<a class="btn btn-xs btn-danger" onclick="del('${var.warnrec_id}');">
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<c:if test="${QX.del == 1}">
															<li>
																<a style="cursor:pointer;" onclick="del('${var.warnrec_id}');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
						...
						<table style="width:100%;">
							<tr>
								...
								<c:if test="${QX.del == 1 }">
								<a class="btn btn-sm btn-danger" onclick="makeAll('确定要删除选中的数据吗?');" >删除</a>
								</c:if>
								</td>
	...
	$(document).keyup(function(event){
		...
		//删除
		function del(warnrec_id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>historywarning/updateDrById.do?warnrec_id="+warnrec_id+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>historywarning/updateAllDr.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
