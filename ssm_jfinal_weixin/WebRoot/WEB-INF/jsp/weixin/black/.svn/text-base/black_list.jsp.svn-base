			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="black/list.do" method="post" name="Form" id="Form">
						<table style="margin-top:5px;">
							<tr>
								...
								<td id="is_black" style="padding-left:2px;">&nbsp;
									<select class="chosen-select form-control" name="is_black"
												id="is_black" data-placeholder="是否在黑名单"
												style="vertical-align:top;" style="width:98%;">
													<!-- <option value="" ></option> -->
													<option value="" <c:if test="${pd.is_black==2 }">selected</c:if>>普通会员</option>
													<option value="0" <c:if test="${pd.is_black==1 }">selected</c:if>>黑名单会员</option>
													<option value="3" <c:if test="${pd.is_black==3 }">selected</c:if>>全部会员</option>
									</select>
								</td>
								<c:if test="${QX.cha == 1 }">		
								...
								<c:if test="${QX.cha == 1 }">
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'><a href="${var.photo}" target="_blank"><img src="${var.photo}" width="50" height="50"/></a></td>											
											<td class='center'>${var.create_date}</td>
											<td class='center'>
												<c:if test="${var.is_black==0 }"><a class="btn btn-sm btn-info">是</a></c:if>
												<c:if test="${var.is_black!=0 }"><a class="btn btn-sm btn-success">否</a></c:if>
											</td>
										</tr>
									</c:forEach>
									</c:if>
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
									...
									<c:if test="${pd.is_black==2 }">
									<a class="btn btn-sm btn-success" onclick="goadd();">拉进黑名单</a>
									</c:if>
									<c:if test="${pd.is_black==1 }">
									<a class="btn btn-sm btn-info" onclick="goremove();">移除黑名单</a>
									</c:if>
									</c:if>
								</td>
	...
	<script type="text/javascript">
		...
		function goadd(){
			...
			for(var i=0;i < document.getElementsByName('ids').length;i++){
			 ...
			 diag.Drag=true;
			 diag.Title ="拉进黑名单";
			 diag.URL = '<%=basePath%>black/goBlack.do?DATA_IDS='+str+'&msg=add';
			 diag.Width = 800;
			 diag.Height = 400;
		...
		function goremove(){
			...
			for(var i=0;i < document.getElementsByName('ids').length;i++){
			 ...
			 diag.Drag=true;
			 diag.Title ="移除黑名单";
			 diag.URL = '<%=basePath%>black/goBlack.do?DATA_IDS='+str+'&msg=remove';
			 diag.Width = 800;
			 diag.Height = 400;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
