			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<input type="hidden" name="parentId" id="parentId" value="${pd.openid}"/>
						<table style="margin-top:5px;">
							<tr>
								...
								<td style="vertical-align:top;padding-left:2px;">
									...
									<option value="1" <c:if test="${pd.chooseType==1 }">selected</c:if>>一级会员</option>
									<option value="2" <c:if test="${pd.chooseType==2 }">selected</c:if>>二级会员</option>
									<option value="3" <c:if test="${pd.chooseType==3 }">selected</c:if>>三级会员</option>
								  	</select>
								</td>
								<td>&nbsp;</td>
									<!-- 金额范围查询 -->
						...
						<br/>
						<a class="btn btn-sm btn-success" style="width:250px;">上级  - ${pd.pName} </a> &nbsp;
						<a class="btn btn-sm btn-success" style="width:100px;" href="${pd.returnstr}?lastStart=${pd.lastStart}&lastEnd=${pd.lastEnd}">返回 </a>
							...
							<c:choose>
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr onclick="showDetail('${var.openid}','${var.orderCount}')">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:0px;">
							...
							<tr>
								...
								<td class='center'>${countPd.saleCount}</td>
								<td class='center'>${countPd.totalOrder}</td>
								<td class='center'>${countPd.totalMoney}</td>
								<td class='center'>${countPd.freeCount}</td>
							</tr>
						</table>						
	    ...
	    if( count > 0 ){
          ...
          if( typeof(time2) == "undefined" ){
            ...
            time2 = time2.substring(0, 10);
             ...
             title = '会员订单明细';
             url =  '<%=basePath%>agentinfo/childdetaillist.do?open_id=' + id +'&lastStart='+time1+'&lastEnd='+time2;
           }else{
             title = '会员个人信息';
             url =  '<%=basePath%>agentinfo/childdetailinfo.do?open_id=' + id ;
           }              
