...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="ord/list.do" method="post" name="Form" id="Form">
	...
	<script type="text/javascript">
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '注意!确定要置为成功吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>ord/updateStatusSuccess.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '注意!确定要置为失败吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>ord/updateStatusFail.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
								//beforeSend: validateData,
								cache: false,
        ...
        var reg = $("#region").val();
			window.location.href='<%=basePath%>ord/excel.do?keywords='+ keyword +'&lastStart='+time1
