			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="ordpress/list.do" method="post" name="Form" id="Form">
						<table style="margin-top:5px;">
							<tr>
								...
								<td style="vertical-align:top;padding-left:20px"">
									<c:if test="${QX.edit == 1 }">
									<a class="btn btn-sm btn-success" style="width:100px;" onclick="refundManay('注意!确定要置为成功吗?');">批量置为成功</a>
									</c:if>
								</td>
								<td style="vertical-align:top;padding-left:20px"">
									<c:if test="${QX.edit == 1 }">
									<a class="btn btn-sm btn-danger" style="width:100px;" onclick="refundManay('注意!确定要置为失败吗?');">批量置为失败</a>
									</c:if>
								</td>
								<td style="vertical-align:top;padding-left:20px"">
									<c:if test="${QX.edit == 1 }">
									<a class="btn btn-sm btn-success" style="width:100px;" onclick="refundManay('注意!确定要充值吗?');">批量充值</a>
									</c:if>
								</td>
							</tr>
						</table>
			...
			bootbox.confirm(msg, function(result) {
				if(result) {
					...
					if(msg == '注意!确定要置为成功吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>ordpress/updateStatusSuccess.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
						...
						}else if(msg == '注意!确定要置为失败吗?'){
							top.jzts();
							$.ajax({
								type: "POST",
								url: '<%=basePath%>ordpress/updateStatusFail.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
								//beforeSend: validateData,
								cache: false,
						...
						}else if(msg == '注意!确定要充值吗?'){
							top.jzts();
							$.ajax({
								type: "POST",
								url: '<%=basePath%>ordpress/doPressRecharge.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
								//beforeSend: validateData,
								cache: false,
...
/*         var paystatus = $("#paystatus").val();
        ...
        var reg = $("#region").val(); */
			window.location.href='<%=basePath%>ordpress/excel.do?keywords='+ keyword +'&lastStart='+time1
			+'&lastEnd='+time2;        
		}
	</script>
