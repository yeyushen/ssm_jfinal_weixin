...
public class AgentInfoController extends BaseController {
	...
	public ModelAndView agentlist(Page page) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"列表AgentInfo");
		...
		if(null != keywords && !"".equals(keywords)){
		    ...
		    strWhere = " AND m.`name` LIKE '%"+ keywords.replace("|", "") +"%' | ";	
		}else{
			strWhere = " |";
		}
		...
		page.setPd(pd);
		pd.put("pro_name", "proc_order_agent_info");
		orderMemberInfoService.createTb(pd);
		List<PageData>	varList = orderMemberInfoService.agentInfoList(page);	//列出QueryInfo列表
		PageData countPd = orderMemberInfoService.countAgentInfo(pd);
		

		pd.put("actionstr", "agentinfo/list.do");
		pd.put("nextaction", "agentinfo/agentalllist.do");
	...
	public ModelAndView agentAllLevelList(Page page) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"列表AgentAllList");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "cha")){return null;} //校验权限(无权查看时页面会有提示,如果不注释掉这句代码就无法进入列表页面,所以根据情况是否加入本句代码)
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		...
		pd.put("actionstr", "agentinfo/agentalllist.do");
		pd.put("returnstr", "agentinfo/list.do");
