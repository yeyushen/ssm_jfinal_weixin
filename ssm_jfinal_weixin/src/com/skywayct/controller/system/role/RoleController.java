package com.skywayct.controller.system.role;
...
public class RoleController extends BaseController {
	...
	/**
	 * 显示订单预警消息列表ztree(订单预警消息授权)
	 * @param model
	 * @return
	 */
	@RequestMapping(value="/newsqx")
	public ModelAndView listAllWarning(Model model,String ROLE_ID)throws Exception{
		ModelAndView mv = this.getModelAndView();
		try{
			Role role = roleService.getRoleById(ROLE_ID);			//根据角色ID获取角色对象
			String roleNewsRights = role.getNEWS_RIGHTS();					//取出本角色订单预警消息权限
			List<OrderWarning> warningList = menuService.listAllNewsQx("0");	//获取所有订单预警消息选项
			warningList = this.readWarning(warningList, roleNewsRights);			//根据角色权限获取对应订单预警消息选项
			JSONArray arr = JSONArray.fromObject(warningList);
			String json = arr.toString();
			json = json.replaceAll("warning_id", "id").replaceAll("parent_id", "pId").replaceAll("warning_tagname", "name").replaceAll("subOrderWarning", "nodes").replaceAll("hasWarning", "checked");
			model.addAttribute("zTreeNodes", json);
			mv.addObject("ROLE_ID",ROLE_ID);
			mv.setViewName("system/role/warningqx");
		} catch(Exception e){
			logger.error(e.toString(), e);
		}
		return mv;
	...
	/**保存角色订单预警消息权限
	 * @param ROLE_ID 角色ID
	 * @param warningIds 订单预警选项id集合
	 * @param out
	 * @throws Exception
	 */
	@RequestMapping(value="/saveNewsqx")
	public void saveNewsqx(@RequestParam String ROLE_ID,@RequestParam String warningIds,PrintWriter out)throws Exception{
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "edit")){} //校验权限
		logBefore(logger, Jurisdiction.getUsername()+"修改菜单权限");
		PageData pd = new PageData();
		try{
			if(null != warningIds && !"".equals(warningIds.trim())){
				BigInteger news_rights = RightsHelper.sumRights(Tools.str2StrArray(warningIds));//用订单预警选项id做权处理
				Role role = roleService.getRoleById(ROLE_ID);	//通过id获取角色对象
				role.setNEWS_RIGHTS(news_rights.toString());
				roleService.updateRoleNewsRights(role);				//更新当前预警消息权限
				pd.put("news_rights",news_rights.toString());
			}else{
				Role role = new Role();
				role.setNEWS_RIGHTS("");
				role.setROLE_ID(ROLE_ID);
				roleService.updateRoleNewsRights(role);				//更新当前角色订单预警消息权限(没有任何勾选)
				pd.put("news_rights","");
			}
				pd.put("ROLE_ID", ROLE_ID);
				roleService.setAllNewsRights(pd);					//更新此角色所有子角色的订单预警消息权限
			out.write("success");
			out.close();
		} catch(Exception e){
			logger.error(e.toString(), e);
	 ...
	 * @return
	 */
	public List<OrderWarning> readWarning(List<OrderWarning> warningList,String roleRights){
		for(int i=0;i<warningList.size();i++){
			warningList.get(i).setHasWarning(RightsHelper.testRights(roleRights, warningList.get(i).getWarning_id()));
		}
		return warningList;
	}
